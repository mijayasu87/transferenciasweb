...
public class PrendaComercialDAO extends DAOAbstract<PrendaComercial> {
    ...
    public boolean existeTramite(String numeroTramite) {
        try {
            Query query = this.getEntityManager().createQuery("Select t from PrendaComercial t where t.solicitud = :tramite");
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
    ...
    public PrendaComercial getPrendaComercialById(Integer id) {
        try {
            Query query = this.getEntityManager().createQuery("Select t from PrendaComercial t where t.id = :idp");
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
