 ...
 * @author micharesp
 */
@ManagedBean(name = "abandonoSubLicBean")
@ViewScoped
public class AbandonoSubLicenciaBean implements Serializable {
    ...
    private List<SubLicenciaUso> abandonos;
    private List<SubLicenciaUso> abandonosFiltradas;
    ...
    private SubLicenciaUso abandono;
    ...
    private List<SubLicenciaUso> selectedAbandonos;
    ...
    public AbandonoSubLicenciaBean() {
        loadAbandonosSubLicencia();
    }

    private void loadAbandonosSubLicencia() {
        Controlador c = new Controlador();
        abandonos = c.getSubLicenciasUsoAbandonoByTipo("ABANDONO");
        numRegistros = "Número Registros Mostrados: " + abandonos.size();
        exportName = "abandono_sublic_" + Operaciones.formatDate(new Date());
    ...
    public void buscarAbandonosLicencia(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            abandonos = c.getSublicenciasUsoByCriteriaAndType(criterio, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
    ...
    public void validarAbandono(SubLicenciaUso licen) {
        FacesMessage msg = null;
        if (licen != null) {
    ...
    public void buscarAbandonosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            abandonos = c.getSublicenciasUsoByFechaAndType(fechaInicio, fechaFin, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarAbandonosPorFechaLicencia(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            abandonos = c.getSublicenciasUsoByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarAbandono(ActionEvent ae) {
        FacesMessage msg = null;
        abandono = (SubLicenciaUso) abandonosDataTable.getRowData();
        if (abandono != null) {
            Controlador c = new Controlador();
            if (c.removeSublicenciaUso(abandono)) {

                c.saveHistorial("ABANDONO_SUBLIC", "ABANDONO_SUBLIC ", abandono.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadAbandonosSubLicencia();
    ...
    public void prepararEditar(ActionEvent ae) {
        ...
        if (abandono != null) {
            Controlador c = new Controlador();
            abandono = c.getSublicenciaUsoBySolicitud(abandono.getSolicitud());
//            c.refreshSubLicenciaUso(abandono);

            roselectable = false;
            dialogTitle = "EDITAR ABANDONO SUBLICENCIA " + abandono.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Abandono_Sublicencia: " + abandono.getSolicitud() + "?";
            RenewalForm rf = c.getRenewalFormsByApplicationNumber(abandono.getSolicitud());
            if (rf.getId() != null) {
                abandono.setIdRenewalForm(rf.getId());
            }
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_SUBLICENCIA CARGADA.");
            PrimeFaces.current().ajax().addCallbackParam("peditar", true);
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "PROBLEMA AL CARGAR ABANDONO_SUBLICENCIA");
    ...
    public void paraProrrogas(ActionEvent ae) {
        ...
        if (selectedAbandonos == null || selectedAbandonos.isEmpty()) {
            ...
            for (int i = 0; i < selectedAbandonos.size(); i++) {
                SubLicenciaUso abaaux = selectedAbandonos.get(i);
                ...
                if (abaaux.getNumeroProrroga() == null) {
                    abaaux.setNumeroProrroga(c.getNextNumeroProrrogaSublicencia(new Date()));
                }
                if (!c.updateSublicenciaUso(abaaux)) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "NO SE PUDO PASAR A PRÓRROGA EL TRÁMITE " + abaaux.getSolicitud());
                    FacesContext.getCurrentInstance().addMessage(null, msg);
                    return;
                }
                c.saveHistorial("PRORROGA_SUBLICENCIA", "ABANDONO_SUBLICENCIA", abaaux.getSolicitud(), "PASADO A PRÓRROGA (" + diasProrroga + " DÍAS)", loginBean.getUsuario().getId(), loginBean.getNombre());
                n++;
            }
            if (n > 0) {
                loadAbandonosSubLicencia();
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO ABANDONO SUBLICENCIA";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar el Nuevo Abandono_SubLicencia?";
        abandono = new SubLicenciaUso();
        ...
        if (abandono != null) {
            ...
            if (abandono.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    ...
                    if (c.updateSublicenciaUso(abandono)) {
                        c.saveHistorial(abandono.getTipoEstado() + "_LICENCIA", "ABANDONO_SUBLICENCIA", abandono.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadAbandonosSubLicencia();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_SUBLICENCIA EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA ABANDONO_SUBLICENCIA");
                    }

                } else {
                    //Editar Abandono
                    if (c.validarExistenciaSublicenciaUso(abandono)) {
                        ...
                        if (c.updateSublicenciaUso(abandono)) {
                            c.saveHistorial("ABANDONO_SUBLICENCIA", "ABANDONO_SUBLICENCIA", abandono.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                //Guardar Abandono sublic
                if (c.existeSublicenciaUso(abandono.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                    ...
                    if (habilitado) {
                        abandono.setNumeroAbandono(c.getNextNumeroAbandonoSublicencia(new Date()));
                        abandono.setSolicitud(abandono.getSolicitud().toUpperCase());
                        abandono.setTipoEstado("ABANDONO");
                        if (c.saveSublicenciaUso(abandono)) {
                            c.saveModificacionApp(abandono.getDenominacion(), abandono.getRegistro(), abandono.getSolicitud(), "SUBLICENCIA DE USO", loginBean.getNombre());
                            c.saveHistorial("ABANDONO_SUBLICENCIA", "ABANDONO_SUBLICENCIA", abandono.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadAbandonosSubLicencia();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                    ...
                    if (abandono.getRegistro() != null && !abandono.getRegistro().trim().isEmpty()) {
                        ...
                        if (abandono.getResponsable() != null && !abandono.getResponsable().trim().isEmpty()) {
                            if (abandono.getCasilleroSenadi() != null && !abandono.getCasilleroSenadi().trim().isEmpty() && !abandono.getCasilleroSenadi().equals("null")) {
                                if (abandono.getSublicenciante() != null && !abandono.getSublicenciante().trim().isEmpty() && !abandono.getSublicenciante().equals("null")) {
                                    if (!c.getRosBySolicitud(abandono.getSolicitud()).isEmpty()) {

                                        abandono.setFechaNotificacion(new Date());
                                        c.updateSublicenciaUso(abandono);
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (abandono != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: ABANDONO_SUBLICENCIA";
            }
        }
    }
                                ...
                                if (!ttp.getAlias().equals("PI") && !ttp.getAlias().equals("MU") && !ttp.getAlias().equals("DI")) {
                                    if (t.getName().trim().toLowerCase().contains("licencia de uso")) {
                                        if (rf.getLicenseType() != null && rf.getLicenseType().equals("SUBLICENSE")) {
                                            ...
                                            abandono.setSublicenciaNo(c.getNextSublicenciaUsoNo());
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
                                            ...
                                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL TRÁMITE " + rf.getApplicationNumber() + " ES UNA LICENCIA");
                                        }
                                    } else {
...
//      RequestContext context = RequestContext.getCurrentInstance();
        if (!selectedAbandonos.isEmpty()) {
            ...
            String msj = "";
            for (int i = 0; i < selectedAbandonos.size(); i++) {
                SubLicenciaUso abandonoaux = selectedAbandonos.get(i);
            ...
            if (band) {
                ...
                loginBean.setSublicencias(selectedAbandonos);
...
//        System.out.println(estadoTemp);
     ...
     */
    public List<SubLicenciaUso> getAbandonos() {
        return abandonos;
    }
     ...
     */
    public void setAbandonos(List<SubLicenciaUso> abandonos) {
     ...
     */
    public List<SubLicenciaUso> getAbandonosFiltradas() {
     ...
     */
    public void setAbandonosFiltradas(List<SubLicenciaUso> abandonosFiltradas) {
     ...
     * @return the abandono
     */
    public SubLicenciaUso getAbandono() {
        return abandono;
    }
     ...
     */
    public void setAbandono(SubLicenciaUso abandono) {
     ...
     */
    public List<SubLicenciaUso> getSelectedAbandonos() {
     ...
     */
    public void setSelectedAbandonos(List<SubLicenciaUso> selectedAbandonos) {
