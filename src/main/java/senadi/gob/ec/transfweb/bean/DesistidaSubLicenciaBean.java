 ...
 * @author Michael
 */
@ManagedBean(name = "desistSublicBean")
@ViewScoped
public class DesistidaSubLicenciaBean implements Serializable {
    ...
    private List<SubLicenciaUso> desistimientos;
    private List<SubLicenciaUso> desistimientosFiltradas;
    ...
    private SubLicenciaUso desistimiento;
    ...
    public DesistidaSubLicenciaBean() {
        loadDesistimientosSublicencia();
    }

    private void loadDesistimientosSublicencia() {
        Controlador c = new Controlador();
        desistimientos = c.getSublicenciasUsoByTipo("DESISTIDA");
        numRegistros = "Número Registros Mostrados: " + desistimientos.size();
        exportName = "desist_sublic_" + Operaciones.formatDate(new Date());
    ...
    public void buscarDesistimiento(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            desistimientos = c.getSublicenciasUsoByCriteriaAndType(criterio, "DESISTIDA");
...
//                                                desistimiento.setTitularAnterior(titulo.getTitular());
                                        ...
                                        if (apoder.getId() != null) {
                                            desistimiento.setApoderadoRepresentante(apoder.getName());
                                                ...
                                                if (rf.getExpedient() != null && !rf.getExpedient().trim().isEmpty()) {
                                                    if (c.existsTituloCanceladoByTituloAndExpediente(desistimiento.getRegistro(), rf.getExpedient(), false)) {
                                                        ...
                                                        if (titca.getId() != null && titca.getTipoCancelacion().contains("TOTAL")) {
                                                            ...
                                                            desistimiento = new SubLicenciaUso();
    ...
    public void buscarDesistimientosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            desistimientos = c.getSublicenciasUsoByFechaAndType(fechaInicio, fechaFin, "DESISTIDA");
            numRegistros = "Número Registros Mostrados: " + desistimientos.size();
            if (desistimientos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarDesistimiento(ActionEvent ae) {
        FacesMessage msg = null;
        desistimiento = (SubLicenciaUso) desistimientoDataTable.getRowData();
        if (desistimiento != null) {
            Controlador c = new Controlador();
            if (c.removeSublicenciaUso(desistimiento)) {
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO DESISTIMIENTO SUBLICENCIA";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar el Nuevo Desistimiento Sublicencia?";
        desistimiento = new SubLicenciaUso();
        Controlador c = new Controlador();
        desistimiento.setResolucionNo(c.getNextSublicenciaUsoResolucionNoByTipo("DESISTIDA"));
            ...
            if (desistimiento.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    ...
                    if (estadoTemp.equals("SUBLICENCIA")) {
                        desistimiento.setTipoEstado("SUBLICENCIA");
                        desistimiento.setSublicenciaNo(c.getNextSublicenciaUsoNo());
                    } else if (estadoTemp.equals("NOTIFICADA")) {
                        desistimiento.setNotificacion(c.getNextNumeroNotificacionSublicenciaUso(new Date()));
                        desistimiento.setTipoEstado("NOTIFICADA");
                    } else if (estadoTemp.equals("TERMINACION")) {
                        desistimiento.setResolucionNo(c.getNextSublicenciaUsoResolucionNoByTipo("TERMINACION"));
                        desistimiento.setTipoEstado("TERMINACION");
                    } else if (estadoTemp.equals("CADUCADA")) {
                        desistimiento.setResolucionNo(c.getNextSublicenciaUsoResolucionNoByTipo("CADUCADA"));
                        desistimiento.setTipoEstado("CADUCADA");
                    }

                    if (c.updateSublicenciaUso(desistimiento)) {
                        c.saveHistorial(desistimiento.getTipoEstado() + "_SUBLICENCIA", "DESISTIDA_SUBLICENCIA", desistimiento.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadDesistimientosSublicencia();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESISTIDA_SUBLICENCIA EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA DESISTIDA_SUBLICENCIA");
                    }
                } else {
                    //Editar Desistimiento
                    if (c.validarExistenciaSublicenciaUso(desistimiento)) {
                        ...
                        if (c.updateSublicenciaUso(desistimiento)) {
                            c.saveHistorial("DESISTIDA_SUBLICENCIA", "DESISTIDA_SUBLICENCIA", desistimiento.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                if (c.existeSublicenciaUso(desistimiento.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        desistimiento.setSolicitud(desistimiento.getSolicitud().toUpperCase());
                        desistimiento.setTipoEstado("DESISTIDA");
                        if (c.saveSublicenciaUso(desistimiento)) {
                            c.saveModificacionApp(desistimiento.getDenominacion(), desistimiento.getRegistro(), desistimiento.getSolicitud(), "SUBLICENCIA DE USO", loginBean.getNombre());
                            c.saveHistorial("DESISTIDA_SUB", "DESISTIDA_SUB", desistimiento.getSolicitud(), "NUEVO DESISTIMIENTO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadDesistimientosSublicencia();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESISTIDA_SUBLICENCIA GUARDADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL GUARDAR LA DESISTIDA_SUBLICENCIA");
                        }
                    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (desistimiento != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: DESISTIDA_SUBLICENCIA";
            }
        }
    }
    ...
    public void viewDesistimiento(ActionEvent ae) {
        ...
        if (desistimiento != null) {
            System.out.println("Descargando Desistimiento Sublicencia: " + desistimiento.getSolicitud());
...
//            loginBean.setDesistimientoFlotante(desistimiento);
     ...
     */
    public List<SubLicenciaUso> getDesistimientos() {
     ...
     */
    public void setDesistimientos(List<SubLicenciaUso> desistimientos) {
     ...
     */
    public List<SubLicenciaUso> getDesistimientosFiltradas() {
     ...
     */
    public void setDesistimientosFiltradas(List<SubLicenciaUso> desistimientosFiltradas) {
     ...
     */
    public SubLicenciaUso getDesistimiento() {
     ...
     */
    public void setDesistimiento(SubLicenciaUso desistimiento) {
