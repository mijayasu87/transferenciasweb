 ...
 * @author micharesp
 */
@ManagedBean(name = "abandonocdBean")
@ViewScoped
public class AbandonoCDBean implements Serializable {
    ...
    private List<CambioDomicilio> abandonos;
    private List<CambioDomicilio> abandonosFiltradas;
    ...
    private CambioDomicilio abandono;
    ...
    private List<CambioDomicilio> selectedAbandonos;
    ...
    private void loadAbandonosCD() {
        Controlador c = new Controlador();
        abandonos = c.getCambiosDomicilioNotificada("ABANDONO");
        numRegistros = "Número Registros Mostrados: " + abandonos.size();
        exportName = "abandono_cd_" + Operaciones.formatDate(new Date());
    ...
    public void buscarAbandonosCD(ActionEvent ae) {
        FacesMessage msg = null;
        if (criterio.trim().isEmpty()) {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "INGRESE UN CRITERIO DE BÚSQUEDA VÁLIDO");
        } else {
            Controlador c = new Controlador();
            abandonos = c.getCambiosDomicilioByCriteriaAndType(criterio, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
    ...
    public void validarAbandono(CambioDomicilio cambion) {
        FacesMessage msg = null;
        if (cambion != null) {
            ...
            if (!uploads.isEmpty()) {
                if (uploads.size() > 1) {
                    for (int i = 0; i < uploads.size(); i++) {
                        ...
                        int conf = Operaciones.validaTextoEnPdf(rutaux, "CERTIFICADO DE CAMBIO DE DOMICILIO DEL");
                        if (conf == 0) {
    ...
    public void buscarAbandonosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            abandonos = c.getCambiosDomicilioByFechaAndType(fechaInicio, fechaFin, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarAbandonosPorFechaCertificado(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            abandonos = c.getCambiosDomicilioByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarAbandono(ActionEvent ae) {
        FacesMessage msg = null;
        abandono = (CambioDomicilio) abandonoDataTable.getRowData();
        if (abandono != null) {
            Controlador c = new Controlador();
            if (c.removeCambioDomicilio(abandono)) {

                c.saveHistorial("ABANDONO_CD", "ABANDONO_CD", abandono.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadAbandonosCD();
...
//            System.out.println("fechaaaaaaaaaA: " + abandono.getFechaPresentacion());
            ...
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_CD CARGADA.");
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO ABANDONO CAMBIO DE DOMICILIO";
//        System.out.println(dialogTitle);
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Abandono_CD?";
        abandono = new CambioDomicilio();
        abandono.setTipoEstado("ABANDONO");
//        Controlador c = new Controlador();
//        abandono.setNotificacion(c.getNextNumeroNotificacionCD(new Date()));
        ...
        if (abandono != null) {
            ...
            if (abandono.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    System.out.println("TipoEstado elegido: " + estadoTemp + ", estaba en: ABANDONO_CD");
                    ...
                    if (c.updateCambioDomicilio(abandono)) {
                        c.saveHistorial(abandono.getTipoEstado() + "_CD", "ABANDONO_CD", abandono.getSolicitud(), "PASADO A", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadAbandonosCD();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_CD EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA ABANDONO_CD");
                    }

                } else {
                    //Editar Abandono
                    if (c.validarExistenciaCambioDomicilio(abandono)) {
                        ...
                        if (c.updateCambioDomicilio(abandono)) {
                            c.saveHistorial("ABANDONO_CD", "ABANDONO_CD", abandono.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                //Guardar Abandono_cd
                if (c.existeCambioDomicilio(abandono.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        abandono.setNumeroAbandono(c.getNextNumeroAbandonoCD(new Date()));
                        abandono.setSolicitud(abandono.getSolicitud().toUpperCase());
                        abandono.setTipoEstado("ABANDONO");
                        if (c.saveCambioDomicilio(abandono)) {
                            c.saveModificacionApp(abandono.getDenominacion(), abandono.getRegistro(), abandono.getSolicitud(), "CAMBIO DE DOMICILIO", loginBean.getNombre());
                            c.saveHistorial("ABANDONO_CD", "ABANDONO_CD", abandono.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadAbandonosCD();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
...
//        System.out.println("Algo tiene que hacer aquí");
        if (abandono != null) {
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (abandono.getNotificacion() != null && abandono.getNotificacion() != 0) {
                    if (!c.getRosBySolicitud(abandono.getSolicitud()).isEmpty()) {
                        ...
                        c.updateCambioDomicilio(abandono);

                        System.out.println("Descargando Abandono_cd: " + abandono.getSolicitud());

                        loginBean.setVarious(false);
                        loginBean.setCambioDomicilio(abandono);

                        PrimeFaces.current().ajax().addCallbackParam("doit", true);

                        System.out.println("envía abandono_cd descargar");
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESCARGANDO ABANDONO_CD " + abandono.getSolicitud());
                    } else {
            ...
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "VALIDE QUE LOS DATOS DE LA ABANDONO_CD SEAN CORRECTOS");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (abandono != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: ABANDONO_CD";
            }
        }
    }
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
    ...
    public void downloadSelected(ActionEvent ae) {
        ...
        if (!selectedAbandonos.isEmpty()) {
            ...
            for (int i = 0; i < selectedAbandonos.size(); i++) {
                CambioDomicilio abandonoaux = selectedAbandonos.get(i);
            ...
            if (band) {
                System.out.println("Descargando Múltiples Abandonos_cd...");
                loginBean.setCambiosDomicilio(selectedAbandonos);
     ...
     */
    public List<CambioDomicilio> getAbandonos() {
        return abandonos;
    }
     ...
     */
    public void setAbandonos(List<CambioDomicilio> abandonos) {
     ...
     */
    public List<CambioDomicilio> getAbandonosFiltradas() {
     ...
     */
    public void setAbandonosFiltradas(List<CambioDomicilio> abandonosFiltradas) {
     ...
     * @return the abandono
     */
    public CambioDomicilio getAbandono() {
        return abandono;
    }
     ...
     */
    public void setAbandono(CambioDomicilio abandono) {
     ...
     */
    public List<CambioDomicilio> getSelectedAbandonos() {
     ...
     */
    public void setSelectedAbandonos(List<CambioDomicilio> selectedAbandonos) {
