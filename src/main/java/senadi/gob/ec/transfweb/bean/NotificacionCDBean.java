 ...
 * @author micharesp
 */
@ManagedBean(name = "notificacioncdBean")
@ViewScoped
public class NotificacionCDBean implements Serializable {
    ...
    private List<CambioDomicilio> notificaciones;
    private List<CambioDomicilio> notificacionesFiltradas;
    ...
    private CambioDomicilio notificacion;
    ...
    private List<CambioDomicilio> selectedNotificaciones;
    ...
    public NotificacionCDBean() {
        loadNotificacionesCD();
    }

    private void loadNotificacionesCD() {
        Controlador c = new Controlador();
        notificaciones = c.getCambiosDomicilioNotificada("NOTIFICADA");
        numRegistros = "Número Registros Mostrados: " + notificaciones.size();
        exportName = "notificacion_cd_" + Operaciones.formatDate(new Date());
    ...
    public void buscarNotificacionesCD(ActionEvent ae) {
        FacesMessage msg = null;
        if (criterio.trim().isEmpty()) {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "INGRESE UN CRITERIO DE BÚSQUEDA VÁLIDO");
        } else {
            Controlador c = new Controlador();
            notificaciones = c.getCambiosDomicilioByCriteriaAndType(criterio, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
    ...
    public void buscarNotificacionesPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            notificaciones = c.getCambiosDomicilioByFechaAndType(fechaInicio, fechaFin, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarNotificacionesPorFechaCertificado(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            notificaciones = c.getCambiosDomicilioByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarNotificacion(ActionEvent ae) {
        FacesMessage msg = null;
        notificacion = (CambioDomicilio) notificacionesDataTable.getRowData();
        if (notificacion != null) {
            Controlador c = new Controlador();
            if (c.removeCambioDomicilio(notificacion)) {
//                c.saveHistorial("RENOVACIÓN", "RENOVACIÓN", renovacion.getSolicitudSenadi(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getUsuario().getLogin());
    ...
    public void validarNotificacion(CambioDomicilio cambion) {
        FacesMessage msg = null;
        if (cambion != null) {
...
//            System.out.println("fechaaaaaaaaaA: " + notificacion.getFechaPresentacion());
            roselectable = false;
            dialogTitle = "EDITAR NOTIFICACIÓN CAMBIO DE DOMICILIO " + notificacion.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Notificación_CD: " + notificacion.getSolicitud() + "?";
            Controlador c = new Controlador();
            RenewalForm rf = c.getRenewalFormsByApplicationNumber(notificacion.getSolicitud());
            if (rf.getId() != null) {
                notificacion.setIdRenewalForm(rf.getId());
            }
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_CD CARGADA.");
            PrimeFaces.current().ajax().addCallbackParam("peditar", true);
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "PROBLEMA AL CARGAR NOTIFICACIÓN_CD");
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVA NOTIFICACIÓN CAMBIO DE DOMICILIO";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Notificación_CD?";
        notificacion = new CambioDomicilio();
        notificacion.setTipoEstado("NOTIFICADA");
        Controlador c = new Controlador();
        notificacion.setNotificacion(c.getNextNumeroNotificacionCD(new Date()));
            ...
            if (notificacion.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    if (estadoTemp.equals("CERTIFICADO")) {
                        ...
                        notificacion.setCertificado(c.getNextCambioDomicilioCertificado());
                        ...
                        notificacion.setTipoEstado("RESOLUCION");
                    }
//                    notificacion.setObservacion(notificacion.getObservacion().equals("NOTIFICADA") ? notificacion.getTipoEstado() : notificacion.getObservacion());

                    if (c.updateCambioDomicilio(notificacion)) {
                        c.saveHistorial(notificacion.getTipoEstado() + "_CD", "NOTIFICADA_CD", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadNotificacionesCD();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICADA_CD EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA NOTIFICADA_CD");
                    }

                } else {
                    //Editar Notificacion
                    if (c.validarExistenciaCambioDomicilio(notificacion)) {
                        ...
                        if (c.updateCambioDomicilio(notificacion)) {
                            c.saveHistorial("NOTIFICADA_CD", "NOTIFICADA_CD", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_CD EDITADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA NOTIFICACIÓN_CD");
                        }
                    }
                }
            } else {
                //Guardar Notificacion_cd
                if (c.existeCambioDomicilio(notificacion.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        notificacion.setSolicitud(notificacion.getSolicitud().toUpperCase());
                        notificacion.setTipoEstado("NOTIFICADA");
                        if (c.saveCambioDomicilio(notificacion)) {
                            c.saveModificacionApp(notificacion.getDenominacion(), notificacion.getRegistro(), notificacion.getSolicitud(), "CAMBIO DE DOMICILIO", loginBean.getNombre());
                            c.saveHistorial("NOTIFICADA_CD", "NOTIFICADA_CD", notificacion.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadNotificacionesCD();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_CD GUARDADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL GUARDAR LA NOTIFICACIÓN_CD");
                        }
                    }
...
//        System.out.println("Algo tiene que hacer aquí");
        if (notificacion != null) {
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (notificacion.getNotificacion() != null && notificacion.getNotificacion() != 0) {
                    ...
                    if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {
                        ...
                        c.updateCambioDomicilio(notificacion);

                        System.out.println("Descargando Notificación_cd: " + notificacion.getSolicitud());

                        loginBean.setVarious(false);
                        loginBean.setCambioDomicilio(notificacion);

                        PrimeFaces.current().ajax().addCallbackParam("doit", true);

                        System.out.println("envía notificación_cd descargar");
            ...
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "VALIDE QUE LOS DATOS DE LA NOTIFICACIÓN_CD SEAN CORRECTOS");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (notificacion.getNotificacion() != null && notificacion.getNotificacion() != 0) {
                    if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {
                        ...
                        System.out.println("envía notificación_cd descargar");
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESCARGANDO NOTIFICACIÓN_CD " + notificacion.getSolicitud());
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL REGISTRO DEBE TENER UN MOTIVO DE NOTIFICACIÓN");
                    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (notificacion != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: NOTIFICADA_CD";
            }
        }
    }
...
//                                                notificacion.setTitularAnterior(titulo.getTitular());
                                                    ...
                                                    if (notificacion.getTitularActual() == null || notificacion.getTitularActual().trim().isEmpty()) {
                                                        PpdiPersona persona = c.getASolicitanteByCodigoSolicitud(ps.getCodigoSolicitudSigno());
                                                        if (persona.getCodigoPersona() != null) {
                                                            notificacion.setTitularActual(persona.getNombrePersona());
                                                            notificacion.setDomicilioTitularActual(persona.getDireccionPersona());
                                                        }
                                                    }
                                                }
                                            }
                                        ...
                                        if (lawyer.getId() != null) {
                                            notificacion.setNomApodRepre(lawyer.getName());
                                        }

                                        if (notificacion.getTitularActual() == null || notificacion.getTitularActual().trim().isEmpty()) {
                                            Person titular = c.getPersonRenewalByType(rf.getId(), "APPLICANT");
                                            if (titular.getId() != null) {
                                                notificacion.setTitularActual(titular.getName());
                                                notificacion.setDomicilioTitularActual(titular.getAddress());
                                            }
                                        }
                                            ...
                                            if (notificacion.getDenominacion() != null && !notificacion.getDenominacion().trim().isEmpty()) {
                                                if (rf.getExpedient() != null && !rf.getExpedient().trim().isEmpty()) {
                                                    if (c.existsTituloCanceladoByTituloAndExpediente(notificacion.getRegistro(), rf.getExpedient(), false)) {
                                                        ...
                                                        if (titca.getId() != null && titca.getTipoCancelacion().contains("TOTAL")) {
                                                            msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL TÍTULO " + notificacion.getRegistro() + " CON DENOMINACIÓN "
                                                                    + notificacion.getDenominacion() + " SE ENCUENTRA CANCELADO; CONSULTE EN EL LISTADO DE TÍTULOS CANCELADOS");
                                                            notificacion = new CambioDomicilio();
                                                        } else if (titca.getId() != null && titca.getTipoCancelacion().contains("PARCIAL")) {
                                                            notificacion.setCancelado(titca.getTipoCancelacion());
                                                            msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL TÍTULO " + notificacion.getRegistro() + " CON DENOMINACIÓN "
                                                                    + notificacion.getDenominacion() + " SE ENCUENTRA CANCELADO PARCIALMENTE; CONSULTE EN EL LISTADO DE TÍTULOS CANCELADOS");
                                                        } else {
                                                            msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL TÍTULO " + notificacion.getRegistro() + " CON DENOMINACIÓN "
                                                                    + notificacion.getDenominacion() + " SE ENCUENTRA CANCELADO; CONSULTE EN EL LISTADO DE TÍTULOS CANCELADOS");
                                                            notificacion = new CambioDomicilio();
...
//      RequestContext context = RequestContext.getCurrentInstance();
        if (!selectedNotificaciones.isEmpty()) {
            ...
            if (band) {
                System.out.println("Descargando Múltiples Notificaciones_cd...");
                loginBean.setVarious(true);
                loginBean.setCambiosDomicilio(selectedNotificaciones);
    ...
    public void prepararDescarga(ActionEvent ae) {
        ...
        notificacion = (CambioDomicilio) notificacionesDataTable.getRowData();
        if (notificacion != null) {
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_CD PREPARADA PARA DESCARGA");
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "NO SE CARGÓ CORRECTAMENTE LA NOTIFICACIÓN_CD");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
    ...
    private String getSolicitudesParaProrroga() {
        String paraProrroga = "";
        for (int i = 0; i < selectedNotificaciones.size(); i++) {
            CambioDomicilio notaux = selectedNotificaciones.get(i);
        ...
        if (selectedNotificaciones.isEmpty()) {
            ...
            if (!noEmitidas.isEmpty()) {
                ...
                for (int i = 0; i < selectedNotificaciones.size(); i++) {
                    CambioDomicilio notaux = selectedNotificaciones.get(i);
                    if (notaux.getFechaPuestaProrroga() != null) {
                        yaProrroga += (yaProrroga.isEmpty() ? "" : ", ") + notaux.getSolicitud();
                    }
                }
        ...
        if (!selectedNotificaciones.isEmpty()) {
            if (tipoAbandono != null && !tipoAbandono.trim().isEmpty()) {
                ...
                for (int i = 0; i < selectedNotificaciones.size(); i++) {
                    ...
                    int nextabandono = c.getNextNumeroAbandonoCD(new Date());
    ...
    public String getTooltipAbandono(CambioDomicilio noti) {
    ...
    public void downloadSelectedErjafe(ActionEvent ae) {
        ...
        if (selectedNotificaciones != null && !selectedNotificaciones.isEmpty()) {
            ...
            String msj = "";
            for (int i = 0; i < selectedNotificaciones.size(); i++) {
                CambioDomicilio notificadaaux = selectedNotificaciones.get(i);
            ...
            if (flag) {
                ...
                loginBean.setTipoTramite(2);
                PrimeFaces.current().ajax().addCallbackParam("doit", true);
                System.out.println("envía notificada cambio domicilio descargar");
     ...
     */
    public List<CambioDomicilio> getNotificaciones() {
     ...
     */
    public void setNotificaciones(List<CambioDomicilio> notificaciones) {
     ...
     */
    public List<CambioDomicilio> getNotificacionesFiltradas() {
     ...
     */
    public void setNotificacionesFiltradas(List<CambioDomicilio> notificacionesFiltradas) {
     ...
     */
    public CambioDomicilio getNotificacion() {
        return notificacion;
    }
     ...
     */
    public void setNotificacion(CambioDomicilio notificacion) {
     ...
     */
    public List<CambioDomicilio> getSelectedNotificaciones() {
     ...
     */
    public void setSelectedNotificaciones(List<CambioDomicilio> selectedNotificaciones) {
