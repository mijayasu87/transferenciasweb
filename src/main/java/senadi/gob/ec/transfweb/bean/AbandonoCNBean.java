 ...
 * @author micharesp
 */
@ManagedBean(name = "abandonocnBean")
@ViewScoped
public class AbandonoCNBean implements Serializable {
    ...
    private List<CambioNombre> abandonos;
    private List<CambioNombre> abandonosFiltradas;
    ...
    private List<CambioNombre> selectedAbandonos;
    ...
    private void loadAbandonosCN() {
        Controlador c = new Controlador();
        abandonos = c.getCambiosNombreNotificada("ABANDONO");
        numRegistros = "Número Registros Mostrados: " + abandonos.size();
        exportName = "abandono_cn_" + Operaciones.formatDate(new Date());
    ...
    public void buscarAbandonosCN(ActionEvent ae) {
        FacesMessage msg = null;
        if (criterio.trim().isEmpty()) {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "INGRESE UN CRITERIO DE BÚSQUEDA VÁLIDO");
        } else {
            Controlador c = new Controlador();
            abandonos = c.getCambiosNombreByCriteriaAndType(criterio, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
    ...
    public void validarAbandono(CambioNombre cambion) {
        FacesMessage msg = null;
        if (cambion != null) {
    ...
    public void buscarAbandonosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            abandonos = c.getCambiosNombreByFechaAndType(fechaInicio, fechaFin, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarAbandonosPorFechaCertificado(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            abandonos = c.getCambiosNombreByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "ABANDONO");
            numRegistros = "Número Registros Mostrados: " + abandonos.size();
            if (abandonos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarAbandono(ActionEvent ae) {
        FacesMessage msg = null;
        abandono = (CambioNombre) abandonoDataTable.getRowData();
        if (abandono != null) {
            Controlador c = new Controlador();
            if (c.removeCambioNombre(abandono)) {

                c.saveHistorial("ABANDONO_CN", "ABANDONO_CN ", abandono.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadAbandonosCN();
...
//            System.out.println("fechaaaaaaaaaA: " + abandono.getFechaPresentacion());
            roselectable = false;
            dialogTitle = "EDITAR ABANDONO CAMBIO DE NOMBRE " + abandono.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Abandono_CN: " + abandono.getSolicitud() + "?";
            RenewalForm rf = c.getRenewalFormsByApplicationNumber(abandono.getSolicitud());
            if (rf.getId() != null) {
                abandono.setIdRenewalForm(rf.getId());
            }
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_CN CARGADA.");
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO ABANDONO CAMBIO DE NOMBRE";
//        System.out.println(dialogTitle);
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Abandono_CN?";
        abandono = new CambioNombre();
        abandono.setTipoEstado("ABANDONO");
//        Controlador c = new Controlador();
//        abandono.setNotificacion(c.getNextNumeroNotificacionCN(new Date()));
        ...
        if (abandono != null) {
            ...
            if (abandono.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    System.out.println("TipoEstado elegido: " + estadoTemp + ", estaba en: ABANDONO_CN");
                    ...
                    if (c.updateCambioNombre(abandono)) {
                        c.saveHistorial(abandono.getTipoEstado() + "_CN", "ABANDONO_CN", abandono.getSolicitud(), "PASADO A", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadAbandonosCN();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "ABANDONO_CN EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA ABANDONO_CN");
                    }

                } else {
                    //Editar Abandono
                    if (c.validarExistenciaCambioNombre(abandono)) {
                        ...
                        if (c.updateCambioNombre(abandono)) {
                            c.saveHistorial("ABANDONO_CN", "ABANDONO_CN", abandono.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                //Guardar Abandono_cn
                if (c.existeCambioNombre(abandono.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        abandono.setNumeroAbandono(c.getNextNumeroAbandonoCN(new Date()));
                        abandono.setSolicitud(abandono.getSolicitud().toUpperCase());
                        abandono.setTipoEstado("ABANDONO");
                        if (c.saveCambioNombre(abandono)) {
                            c.saveModificacionApp(abandono.getDenominacion(), abandono.getRegistro(), abandono.getSolicitud(), "CAMBIO DE NOMBRE", loginBean.getNombre());
                            c.saveHistorial("ABANDONO_CN", "ABANDONO_CN", abandono.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadAbandonosCN();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
...
//        System.out.println("Algo tiene que hacer aquí");
        if (abandono != null) {
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (abandono.getNotificacion() != null && abandono.getNotificacion() != 0) {
                    if (!c.getRosBySolicitud(abandono.getSolicitud()).isEmpty()) {
                        ...
                        c.updateCambioNombre(abandono);

                        System.out.println("Descargando Abandono_cn: " + abandono.getSolicitud());
                        ...
                        System.out.println("envía abandono_cn descargar");
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESCARGANDO ABANDONO_CN " + abandono.getSolicitud());
                    } else {
            ...
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "VALIDE QUE LOS DATOS DE LA ABANDONO_CN SEAN CORRECTOS");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (abandono != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: ABANDONO_CN";
            }
        }
    }
                            ...
                            if (t.getId() != null && ttp.getId() != null) {
                                if (!ttp.getAlias().equals("PI") && !ttp.getAlias().equals("MU") && !ttp.getAlias().equals("DI")) {
                                    if (t.getName().trim().toLowerCase().contains("cambio de nombre")) {
                                        ...
                                        abandono.setCertificado(c.getNextCambioNombreCertificado());
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
    ...
    public void downloadSelected(ActionEvent ae) {
        ...
        if (!selectedAbandonos.isEmpty()) {
            ...
            for (int i = 0; i < selectedAbandonos.size(); i++) {
                CambioNombre abandonoaux = selectedAbandonos.get(i);
            ...
            if (band) {
                System.out.println("Descargando Múltiples Abandonos_cn...");
                loginBean.setCambiosNombre(selectedAbandonos);
     ...
     */
    public List<CambioNombre> getAbandonos() {
        return abandonos;
    }
     ...
     */
    public void setAbandonos(List<CambioNombre> abandonos) {
     ...
     */
    public List<CambioNombre> getAbandonosFiltradas() {
     ...
     */
    public void setAbandonosFiltradas(List<CambioNombre> abandonosFiltradas) {
     ...
     * @return the abandono
     */
    public CambioNombre getAbandono() {
        return abandono;
    }
     ...
     */
    public void setAbandono(CambioNombre abandono) {
     ...
     */
    public List<CambioNombre> getSelectedAbandonos() {
     ...
     */
    public void setSelectedAbandonos(List<CambioNombre> selectedAbandonos) {
