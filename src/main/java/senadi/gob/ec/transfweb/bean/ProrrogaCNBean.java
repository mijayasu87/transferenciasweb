 ...
 * @author Michael
 */
@ManagedBean(name = "prorrogacnBean")
@ViewScoped
public class ProrrogaCNBean implements Serializable {
    ...
    private List<CambioNombre> prorrogas;
    private List<CambioNombre> prorrogasFiltradas;
    ...
    private CambioNombre prorroga;

    private String estadoTemp;

    private List<CambioNombre> selectedProrrogas;
    ...
    public ProrrogaCNBean() {
        loadProrrogasCN();
        selectedProrrogas = new ArrayList<>();
    }

    public void validarProrroga(CambioNombre cambion) {
    ...
    private boolean faltanDatosAlcance(CambioNombre p) {
    ...
    public void viewProrroga(ActionEvent ae) {
        FacesMessage msg = null;
        prorroga = (CambioNombre) prorrogaDataTable.getRowData();
        if (prorroga != null && prorroga.getId() != null) {
            if (faltanDatosAlcance(prorroga)) {
                ...
                loginBean.setCambioNombre(prorroga);
                loginBean.setCambiosNombre(new ArrayList<CambioNombre>());
    ...
    public void downloadSelected(ActionEvent ae) {
        ...
        if (selectedProrrogas != null && !selectedProrrogas.isEmpty()) {
            ...
            for (int i = 0; i < selectedProrrogas.size(); i++) {
                CambioNombre p = selectedProrrogas.get(i);
            ...
            if (!faltan.isEmpty()) {
                ...
                loginBean.setCambioNombre(null);
                loginBean.setCambiosNombre(selectedProrrogas);
    ...
    private void loadProrrogasCN() {
        Controlador c = new Controlador();
        prorrogas = c.getCambiosNombreByTipo("PRORROGA");
        numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
        exportName = "prorroga_cn_" + Operaciones.formatDate(new Date());
        loginBean = c.getLogin();
    }

    public boolean isProrrogaVencida(CambioNombre p) {
    ...
    public String getEstiloProrroga(CambioNombre p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
    ...
    public String getDiasRestantes(CambioNombre p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
        LocalDate limite = Operaciones.calcularFechaLimiteProrroga(p.getSolicitud(), p.getFechaPuestaProrroga(), p.getDiasProrroga());
    ...
    public String getTooltipProrroga(CambioNombre p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
        LocalDate limite = Operaciones.calcularFechaLimiteProrroga(p.getSolicitud(), p.getFechaPuestaProrroga(), p.getDiasProrroga());
    ...
    public void buscarProrroga(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            prorrogas = c.getCambiosNombreByCriteriaAndType(criterio, "PRORROGA");
            numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
    ...
    public void buscarProrrogasPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            prorrogas = c.getCambiosNombreByFechaAndType(fechaInicio, fechaFin, "PRORROGA");
            numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
            if (prorrogas.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
    ...
    public void eliminarProrroga(ActionEvent ae) {
        ...
        if (prorroga != null) {
            Controlador c = new Controlador();
            if (c.removeCambioNombre(prorroga)) {
                loadProrrogasCN();
    ...
    public void guardarProrroga(ActionEvent ae) {
        ...
        if (prorroga != null && prorroga.getId() != null) {
            ...
            if (c.validarExistenciaCambioNombre(prorroga)) {
                ...
                if (prorroga.getNumeroProrroga() == null) {
                    prorroga.setNumeroProrroga(c.getNextNumeroProrrogaCN(new Date()));
                }
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    if (estadoTemp.equals("CERTIFICADO")) {
                        prorroga.setTipoEstado("CERTIFICADO");
                        prorroga.setCertificado(c.getNextCambioNombreCertificado());
                        ...
                        if (prorroga.getNumeroAbandono() == null) {
                            prorroga.setNumeroAbandono(c.getNextNumeroAbandonoCN(new Date()));
                        }
                    } else {
                        prorroga.setTipoEstado("NOTIFICADA");
                    }
                }
                if (c.updateCambioNombre(prorroga)) {
                    c.saveHistorial(estadoTemp != null && !estadoTemp.trim().isEmpty() ? prorroga.getTipoEstado() + "_CN" : "PRORROGA_CN", "PRORROGA_CN", prorroga.getSolicitud(), estadoTemp != null && !estadoTemp.trim().isEmpty() ? "PASADO A " + prorroga.getTipoEstado() : "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                    loadProrrogasCN();
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (prorroga != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: PRORROGA_CN";
            }
        }
    }
    ...
    public List<CambioNombre> getProrrogas() {
        return prorrogas;
    }

    public void setProrrogas(List<CambioNombre> prorrogas) {
        this.prorrogas = prorrogas;
    }

    public List<CambioNombre> getProrrogasFiltradas() {
        return prorrogasFiltradas;
    }

    public void setProrrogasFiltradas(List<CambioNombre> prorrogasFiltradas) {
    ...
    public CambioNombre getProrroga() {
        return prorroga;
    }

    public void setProrroga(CambioNombre prorroga) {
    ...
    public List<CambioNombre> getSelectedProrrogas() {
        return selectedProrrogas;
    }

    public void setSelectedProrrogas(List<CambioNombre> selectedProrrogas) {
