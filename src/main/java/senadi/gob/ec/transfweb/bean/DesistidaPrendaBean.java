 ...
 * @author Michael
 */
@ManagedBean(name = "desistidaPrendaBean")
@ViewScoped
public class DesistidaPrendaBean implements Serializable {
    ...
    private List<PrendaComercial> desistimientos;
    private List<PrendaComercial> desistimientosFiltradas;
    ...
    private PrendaComercial desistimiento;
    ...
    public DesistidaPrendaBean() {
        loadDesistimientosPrendaComercial();
    }

    private void loadDesistimientosPrendaComercial() {
        Controlador c = new Controlador();
        desistimientos = c.getPrendasComercialesByTipo("DESISTIDA");
        numRegistros = "Número Registros Mostrados: " + desistimientos.size();
        exportName = "desistimiento_cn_" + Operaciones.formatDate(new Date());
    ...
    public void buscarDesistimiento(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            desistimientos = c.getPrendasComercialesByCriteriaAndType(criterio, "DESISTIDA");
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
                                    ...
                                    if (abog.getId() != null) {
                                        desistimiento.setAbogadoPatrocinador(abog.getName());
                                        desistimiento.setEmail(abog.getEmail());
                                    }

                                    Person apoder = c.getFirstPersonRenewalTypeByIdRenewal(rf.getId(), "AGENT");
                                    if (apoder.getId() != null) {
                                        desistimiento.setTitApodRepre(c.getNamesPersonRenewalTextTypeByIdRenewal(rf.getId(), "AGENT"));
                                    }

                                    Person acreed = c.getFirstPersonRenewalTypeByIdRenewal(rf.getId(), "BENEFICIARY");
                                    if (acreed.getId() != null) {
                                        desistimiento.setPrendariaAcreedora(acreed.getName());
                                    }
                                    Person deudP = c.getFirstPersonRenewalTypeByIdRenewal(rf.getId(), "APPLICANT");
                                    if (deudP.getId() != null) {
                                        desistimiento.setDeudoraPrendaria(deudP.getName());
                                    }
                                    if ((desistimiento.getRegistro() == null || desistimiento.getRegistro().trim().isEmpty()) && rf.getTransactionNumber() != null && !rf.getTransactionNumber().trim().isEmpty()) {
                                        desistimiento.setRegistro(rf.getTransactionNumber());
                                    }
                                            ...
                                            if (rf.getExpedient() != null && !rf.getExpedient().trim().isEmpty()) {
                                                if (c.existsTituloCanceladoByTituloAndExpediente(desistimiento.getRegistro(), rf.getExpedient(), false)) {
                                                    ...
                                                    if (titca.getId() != null && titca.getTipoCancelacion().contains("TOTAL")) {
                                                        ...
                                                        desistimiento = new PrendaComercial();
                                                ...
                                                if (c.existsTituloCanceladoByTituloAndDenominacion(desistimiento.getRegistro(), desistimiento.getDenominacion(), false)) {
                                                    ...
                                                    if (titca.getId() != null && titca.getTipoCancelacion().contains("TOTAL")) {
                                                        msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL TÍTULO " + desistimiento.getRegistro() + " CON DENOMINACIÓN "
                                                                + desistimiento.getDenominacion() + " SSE ENCUENTRA CANCELADO DE MANERA " + titca.getTipoCancelacion() + "; CONSULTE EN EL LISTADO DE TÍTULOS CANCELADOS");
                                                        desistimiento = new PrendaComercial();
                                                    } else if (titca.getId() != null && titca.getTipoCancelacion().contains("PARCIAL")) {
                                                        desistimiento.setCancelado(titca.getTipoCancelacion());
                                                        msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL TÍTULO " + desistimiento.getRegistro() + " CON DENOMINACIÓN "
                                    ...
                                    msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "TRÁMITE ENCONTRADO PERO NO ES UNA DESISTIDA_CN, SINO " + t.getName().toUpperCase());
                                }
                            } else {
    ...
    public void buscarDesistimientosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            desistimientos = c.getPrendasComercialesByFechaAndType(fechaInicio, fechaFin, "DESISTIDA");
            numRegistros = "Número Registros Mostrados: " + desistimientos.size();
            if (desistimientos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarDesistimiento(ActionEvent ae) {
        FacesMessage msg = null;
        desistimiento = (PrendaComercial) desistimientoDataTable.getRowData();
        if (desistimiento != null) {
            Controlador c = new Controlador();
            if (c.removePrendaComercial(desistimiento)) {
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO DESISTIMIENTO PRENDA COMERCIAL";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar el Nuevo Desistimiento Prenda Comercial?";
        desistimiento = new PrendaComercial();
        Controlador c = new Controlador();
        desistimiento.setResolucionNo(c.getNextNumeroResolucionDesistimientoPrenda());
            ...
            if (desistimiento.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    ...
                    if (estadoTemp.equals("CERTIFICADO")) {
                        ...
                        desistimiento.setPrendaNo(c.getNextPrendaComercialPrendaNo());
                        desistimiento.setFechaPrenda(Operaciones.cambiarFechaADiaDado(new Date(), 5));
                    } else if (estadoTemp.equals("NOTIFICADA")) {
                        desistimiento.setTipoEstado("NOTIFICADA");
                        desistimiento.setFechaNotificacion(new Date());
                        desistimiento.setNotificacion(c.getNextNumeroNotificacionPrendaComercial(new Date()));
                    } else if (estadoTemp.equals("LEVANTAMIENTO")) {
                        desistimiento.setTipoEstado("LEVANTAMIENTO");
                        desistimiento.setPrendaNo(c.getNextNumeroLevantamientoPrenda(new Date()));
                    } else if (estadoTemp.equals("DESISTIDAS")) {
                        desistimiento.setTipoEstado("DESISITDA");
                    } else {
                        desistimiento.setTipoEstado("CADUCADA");
                    }

                    if (c.updatePrendaComercial(desistimiento)) {
                        c.saveHistorial(desistimiento.getTipoEstado() + "_PRENDA", "DESISTIDA_PRENDA", desistimiento.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadDesistimientosPrendaComercial();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESISTIDA_CN EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA DESISTIDA_PRENDA");
                    }
                } else {
                    //Editar Desistimiento
                    if (c.validarExistenciaPrendaComercial(desistimiento)) {
                        ...
                        if (c.updatePrendaComercial(desistimiento)) {
                            c.saveHistorial("DESISTIDA_CN", "DESISTIDA_CN", desistimiento.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                if (c.existePrendaComercial(desistimiento.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        desistimiento.setSolicitud(desistimiento.getSolicitud().toUpperCase());
                        desistimiento.setTipoEstado("DESISTIDA");
                        if (c.savePrendaComercial(desistimiento)) {
                            c.saveModificacionApp(desistimiento.getDenominacion(), desistimiento.getRegistro(), desistimiento.getSolicitud(), "PRENDA COMERCIAL", loginBean.getNombre());
                            ...
                            loadDesistimientosPrendaComercial();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESISTIDA_CN GUARDADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL GUARDAR LA DESISTIDA_PRENDA");
                        }
                    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (desistimiento != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: DESISTIDA_PRENDA";
            }
        }
    }
...
//            loginBean.setDesistimientoFlotante(desistimiento);
     ...
     */
    public List<PrendaComercial> getDesistimientos() {
     ...
     */
    public void setDesistimientos(List<PrendaComercial> desistimientos) {
     ...
     */
    public List<PrendaComercial> getDesistimientosFiltradas() {
     ...
     */
    public void setDesistimientosFiltradas(List<PrendaComercial> desistimientosFiltradas) {
     ...
     */
    public PrendaComercial getDesistimiento() {
     ...
     */
    public void setDesistimiento(PrendaComercial desistimiento) {
