 ...
 * @author Michael
 */
@ManagedBean(name = "desistidacnBean")
@ViewScoped
public class DesistimientoCNBean implements Serializable {
    ...
    private List<CambioNombre> desistimientos;
    private List<CambioNombre> desistimientosFiltradas;
    ...
    private CambioNombre desistimiento;
    ...
    public DesistimientoCNBean() {
        loadDesistimientosCN();
    }

    private void loadDesistimientosCN() {
        Controlador c = new Controlador();
        desistimientos = c.getCambiosNombreByTipo("DESISTIDA");
    ...
    public void buscarDesistimiento(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            desistimientos = c.getCambiosNombreByCriteriaAndType(criterio, "DESISTIDA");
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
    ...
    public void buscarDesistimientosPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            desistimientos = c.getCambiosNombreByFechaAndType(fechaInicio, fechaFin, "DESISTIDA");
            numRegistros = "Número Registros Mostrados: " + desistimientos.size();
            if (desistimientos.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarDesistimiento(ActionEvent ae) {
        FacesMessage msg = null;
        desistimiento = (CambioNombre) desistimientoDataTable.getRowData();
        if (desistimiento != null) {
            Controlador c = new Controlador();
            if (c.removeCambioNombre(desistimiento)) {
//                c.saveHistorial("RENOVACIÓN", "RENOVACIÓN", renovacion.getSolicitudSenadi(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getUsuario().getLogin());
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO DESISTIMIENTO CAMBIO DE NOMBRE";
        saveEdit = "GUARDAR";
            ...
            if (desistimiento.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    ...
                    if (estadoTemp.equals("CERTIFICADO")) {
                        ...
                        desistimiento.setCertificado(c.getNextCambioNombreCertificado());
                    ...
                    if (c.updateCambioNombre(desistimiento)) {
                        c.saveHistorial(desistimiento.getTipoEstado() + "_CN", "DESISTIDA_CN", desistimiento.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadDesistimientosCN();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESISTIDA_CN EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA DESISTIDA_CN");
                    }
                } else {
                    //Editar Desistimiento
                    if (c.validarExistenciaCambioNombre(desistimiento)) {
                ...
                if (c.existeCambioNombre(desistimiento.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        desistimiento.setSolicitud(desistimiento.getSolicitud().toUpperCase());
                        desistimiento.setTipoEstado("DESISTIDA");
                        if (c.saveCambioNombre(desistimiento)) {
                            c.saveModificacionApp(desistimiento.getDenominacion(), desistimiento.getRegistro(), desistimiento.getSolicitud(), "CAMBIO DE NOMBRE", loginBean.getNombre());
                            ...
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL GUARDAR LA DESISTIDA_CN");
                        }
                    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (desistimiento != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: DESISTIDA_CN";
            }
        }
    }
...
//            loginBean.setDesistimientoFlotante(desistimiento);
     ...
     */
    public List<CambioNombre> getDesistimientos() {
     ...
     */
    public void setDesistimientos(List<CambioNombre> desistimientos) {
     ...
     */
    public List<CambioNombre> getDesistimientosFiltradas() {
     ...
     */
    public void setDesistimientosFiltradas(List<CambioNombre> desistimientosFiltradas) {
     ...
     */
    public CambioNombre getDesistimiento() {
     ...
     */
    public void setDesistimiento(CambioNombre desistimiento) {
