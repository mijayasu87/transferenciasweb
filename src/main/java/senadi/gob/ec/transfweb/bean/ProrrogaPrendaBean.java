 ...
 * @author Michael
 */
@ManagedBean(name = "prorrogaprendaBean")
@ViewScoped
public class ProrrogaPrendaBean implements Serializable {
    ...
    private List<PrendaComercial> prorrogas;
    private List<PrendaComercial> prorrogasFiltradas;
    ...
    private PrendaComercial prorroga;

    private String estadoTemp;

    private List<PrendaComercial> selectedProrrogas;
    ...
    public ProrrogaPrendaBean() {
        loadProrrogasPrenda();
        selectedProrrogas = new ArrayList<>();
    }

    public void validarProrroga(PrendaComercial cambion) {
    ...
    private boolean faltanDatosAlcance(PrendaComercial p) {
    ...
    public void viewProrroga(ActionEvent ae) {
        FacesMessage msg = null;
        prorroga = (PrendaComercial) prorrogaDataTable.getRowData();
        if (prorroga != null && prorroga.getId() != null) {
            if (faltanDatosAlcance(prorroga)) {
                ...
                loginBean.setPrendaComercial(prorroga);
    ...
    public void downloadSelected(ActionEvent ae) {
        ...
        if (selectedProrrogas != null && !selectedProrrogas.isEmpty()) {
            ...
            for (int i = 0; i < selectedProrrogas.size(); i++) {
                PrendaComercial p = selectedProrrogas.get(i);
            ...
            if (!faltan.isEmpty()) {
                ...
                loginBean.setPrendaComercial(null);
                loginBean.setPrendasComerciales(selectedProrrogas);
    ...
    private void loadProrrogasPrenda() {
        Controlador c = new Controlador();
        prorrogas = c.getPrendasComercialesByTipo("PRORROGA");
        numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
        exportName = "prorroga_prenda_" + Operaciones.formatDate(new Date());
        loginBean = c.getLogin();
    }

    public boolean isProrrogaVencida(PrendaComercial p) {
    ...
    public String getEstiloProrroga(PrendaComercial p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
    ...
    public String getDiasRestantes(PrendaComercial p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
        LocalDate limite = Operaciones.calcularFechaLimiteProrroga(p.getSolicitud(), p.getFechaPuestaProrroga(), p.getDiasProrroga());
    ...
    public String getTooltipProrroga(PrendaComercial p) {
        if (p == null || p.getFechaPuestaProrroga() == null || p.getDiasProrroga() == null) {
            return "";
        }
        LocalDate limite = Operaciones.calcularFechaLimiteProrroga(p.getSolicitud(), p.getFechaPuestaProrroga(), p.getDiasProrroga());
    ...
    public void buscarProrroga(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            prorrogas = c.getPrendasComercialesByCriteriaAndType(criterio, "PRORROGA");
            numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
    ...
    public void buscarProrrogasPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            prorrogas = c.getPrendasComercialesByFechaAndType(fechaInicio, fechaFin, "PRORROGA");
            numRegistros = "Número Registros Mostrados: " + prorrogas.size();
        calcularAlertaVencidas();
            if (prorrogas.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
    ...
    public void eliminarProrroga(ActionEvent ae) {
        ...
        if (prorroga != null) {
            Controlador c = new Controlador();
            if (c.removePrendaComercial(prorroga)) {
                loadProrrogasPrenda();
    ...
    public void guardarProrroga(ActionEvent ae) {
        ...
        if (prorroga != null && prorroga.getId() != null) {
            ...
            if (c.validarExistenciaPrendaComercial(prorroga)) {
                ...
                if (prorroga.getNumeroProrroga() == null) {
                    prorroga.setNumeroProrroga(c.getNextNumeroProrrogaPrenda(new Date()));
                }
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    if (estadoTemp.equals("CERTIFICADO")) {
                        prorroga.setTipoEstado("CERTIFICADO");
                        prorroga.setPrendaNo(c.getNextPrendaComercialPrendaNo());
                        ...
                        if (prorroga.getNumeroAbandono() == null) {
                            prorroga.setNumeroAbandono(c.getNextNumeroAbandonoPrenda(new Date()));
                        }
                    } else {
                        prorroga.setTipoEstado("NOTIFICADA");
                    }
                }
                if (c.updatePrendaComercial(prorroga)) {
                    c.saveHistorial(estadoTemp != null && !estadoTemp.trim().isEmpty() ? prorroga.getTipoEstado() + "_PRENDA" : "PRORROGA_PRENDA", "PRORROGA_PRENDA", prorroga.getSolicitud(), estadoTemp != null && !estadoTemp.trim().isEmpty() ? "PASADO A " + prorroga.getTipoEstado() : "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                    loadProrrogasPrenda();
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (prorroga != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: PRORROGA_PRENDA";
            }
        }
    }
    ...
    public List<PrendaComercial> getProrrogas() {
        return prorrogas;
    }

    public void setProrrogas(List<PrendaComercial> prorrogas) {
        this.prorrogas = prorrogas;
    }

    public List<PrendaComercial> getProrrogasFiltradas() {
        return prorrogasFiltradas;
    }

    public void setProrrogasFiltradas(List<PrendaComercial> prorrogasFiltradas) {
    ...
    public PrendaComercial getProrroga() {
        return prorroga;
    }

    public void setProrroga(PrendaComercial prorroga) {
    ...
    public List<PrendaComercial> getSelectedProrrogas() {
        return selectedProrrogas;
    }

    public void setSelectedProrrogas(List<PrendaComercial> selectedProrrogas) {
