 ...
 * @author micharesp
 */
@ManagedBean(name = "notificacionLicenciaBean")
@ViewScoped
public class NotificacionLicenciaBean implements Serializable {
    ...
    private List<LicenciaUso> notificaciones;
    private List<LicenciaUso> notificacionesFiltradas;
    ...
    private LicenciaUso notificacion;
    ...
    private List<LicenciaUso> selectedNotificaciones;
    ...
    private void loadNotificacionesLicencia() {
        Controlador c = new Controlador();
        notificaciones = c.getLicenciasUsoNotificada("NOTIFICADA");
        numRegistros = "Número Registros Mostrados: " + notificaciones.size();
        exportName = "not_licencia_" + Operaciones.formatDate(new Date());
    ...
    public void buscarNotificacionesLicencia(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            notificaciones = c.getLicenciasUsoByCriteriaAndType(criterio, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
    ...
    public void validarNotificacion(LicenciaUso licen) {
        FacesMessage msg = null;
        if (licen != null) {
    ...
    public void buscarNotificacionesPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            notificaciones = c.getLicenciasUsoByFechaAndType(fechaInicio, fechaFin, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarNotificacionesPorFechaLicencia(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            notificaciones = c.getLicenciasUsoByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarNotificacion(ActionEvent ae) {
        FacesMessage msg = null;
        notificacion = (LicenciaUso) notificacionesDataTable.getRowData();
        if (notificacion != null) {
            Controlador c = new Controlador();
            if (c.removeLicenciaUso(notificacion)) {

                c.saveHistorial("NOTIFICADA_LICENCIA", "NOTIFICADA_LICENCIA ", notificacion.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadNotificacionesLicencia();
...
//            System.out.println("fechaaaaaaaaaA: " + notificacion.getFechaPresentacion());
            roselectable = false;
            dialogTitle = "EDITAR NOTIFICACIÓN LICENCIA " + notificacion.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Notificación_Licencia: " + notificacion.getSolicitud() + "?";
            Controlador c = new Controlador();
            RenewalForm rf = c.getRenewalFormsByApplicationNumber(notificacion.getSolicitud());
            if (rf.getId() != null) {
                notificacion.setIdRenewalForm(rf.getId());
            }
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_LICENCIA CARGADA.");
            PrimeFaces.current().ajax().addCallbackParam("peditar", true);
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "PROBLEMA AL CARGAR NOTIFICACIÓN_LICENCIA");
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVA NOTIFICACIÓN LICENCIA";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Notificación_Licencia?";
        notificacion = new LicenciaUso();
        notificacion.setTipoEstado("NOTIFICADA");
        Controlador c = new Controlador();
        notificacion.setNotificacion(c.getNextNumeroNotificacionLicenciaUso(new Date()));
            ...
            if (notificacion.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    if (estadoTemp.equals("LICENCIA")) {
                        notificacion.setTipoEstado("LICENCIA");
                        notificacion.setFechaLicencia(Operaciones.cambiarFechaADiaDado(new Date(), 5));
                        notificacion.setLicenciaNo(c.getNextLicenciaUsoNo());
                    } else if (estadoTemp.equals("TERMINACION")) {
                        notificacion.setTerminacionNo(c.getNextLicenciaTerminacionNo(new Date()));
                        notificacion.setFechaTerminacion(new Date());
                        notificacion.setTipoEstado("TERMINACION");
                    } else if (estadoTemp.equals("DESISTIDAS")) {
                        notificacion.setResolucionNo(c.getNextLicenciaUsoResolucionNoByTipo("DESISTIDA"));
                        notificacion.setTipoEstado("DESISTIDA");
                    } else if (estadoTemp.equals("CADUCADAS")) {
                        notificacion.setResolucionCaducada(c.getNextLicenciaUsoResolucionCaducadaNo("CADUCADA"));
                        notificacion.setFechaResolucionCaducada(new Date());
                        notificacion.setTipoEstado("CADUCADA");
                    }

                    if (c.updateLicenciaUso(notificacion)) {
                        c.saveHistorial(notificacion.getTipoEstado() + "_LICENCIA", "NOTIFICADA_LICENCIA", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadNotificacionesLicencia();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICADA_LICENCIA EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA NOTIFICADA_LICENCIA");
                    }

                } else {
                    //Editar Notificacion
                    if (c.validarExistenciaLicenciaUso(notificacion)) {
                        ...
                        if (c.updateLicenciaUso(notificacion)) {
                            c.saveHistorial("NOTIFICADA_LICENCIA", "NOTIFICADA_LICENCIA", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                if (c.existeLicenciaUso(notificacion.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else if (c.existeSublicenciaUso(notificacion.getSolicitud())) {
                    ...
                    if (habilitado) {
                        notificacion.setSolicitud(notificacion.getSolicitud().toUpperCase());
                        notificacion.setTipoEstado("NOTIFICADA");
                        if (c.saveLicenciaUso(notificacion)) {
                            c.saveModificacionApp(notificacion.getDenominacion(), notificacion.getRegistro(), notificacion.getSolicitud(), "LICENCIA DE USO", loginBean.getNombre());
                            c.saveHistorial("NOTIFICADA_LICENCIA", "NOTIFICADA_LICENCIA", notificacion.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadNotificacionesLicencia();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (notificacion.getNotificacion() != null && !notificacion.getNotificacion().trim().isEmpty()) {
                    if (notificacion.getRegistro() != null && !notificacion.getRegistro().trim().isEmpty()) {
                        ...
                        if (notificacion.getResponsable() != null && !notificacion.getResponsable().trim().isEmpty()) {
                            if (notificacion.getCasilleroSenadi() != null && !notificacion.getCasilleroSenadi().trim().isEmpty() && !notificacion.getCasilleroSenadi().equals("null")) {
                                if (notificacion.getLicenciante() != null && !notificacion.getLicenciante().trim().isEmpty() && !notificacion.getLicenciante().equals("null")) {
                                    if ((notificacion.getCasilleroSenadiLicenciatario() != null && !notificacion.getCasilleroSenadiLicenciatario().trim().isEmpty())
                                            || (notificacion.getEmail() != null && !notificacion.getEmail().trim().isEmpty())) {
                                        if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {

                                            notificacion.setFechaNotificacion(new Date());
                                            c.updateLicenciaUso(notificacion);
                                            System.out.println("Descargando Notificación_licencia: " + notificacion.getSolicitud());
//                                            correcto = true;
                                            loginBean.setVarious(false);
                                            loginBean.setLicencia(notificacion);
                                        ...
                                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL REGISTRO " + notificacion.getSolicitud() + " DEBE TENER UN CASILLERO LICENCIATARIO O UN CORREO");
                                    }
                                } else {
                                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL REGISTRO " + notificacion.getSolicitud() + " NO TIENE LICENCIANTE");
                                }
                            } else {
                                msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL REGISTRO " + notificacion.getSolicitud() + " NO TIENE CASILLERO");
                            }
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL REGISTRO " + notificacion.getSolicitud() + " NO TIENE RESPONSABLE ASIGNADO");
                        }
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "EL REGISTRO " + notificacion.getSolicitud() + " NO POSEE NÚMERO DE REGISTRO");
                    }
                } else {
                ...
                if (notificacion.getNotificacion() != null && !notificacion.getNotificacion().trim().isEmpty()) {
                    if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {
                        ...
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESCARGANDO NOTIFICACIÓN_LICENCIA " + notificacion.getSolicitud());
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "EL REGISTRO DEBE TENER UN MOTIVO DE NOTIFICACIÓN");
                    }
                } else {
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (notificacion != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: NOTIFICADA_LICENCIA";
            }
        }
    }
...
//      RequestContext context = RequestContext.getCurrentInstance();
        if (!selectedNotificaciones.isEmpty()) {
            ...
            if (band) {
                System.out.println("Descargando Múltiples Notificaciones_Licencia...");
                loginBean.setVarious(true);
                loginBean.setLicencias(selectedNotificaciones);
    ...
    public void prepararDescarga(ActionEvent ae) {
        ...
        notificacion = (LicenciaUso) notificacionesDataTable.getRowData();
        if (notificacion != null) {
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_LICENCIA PREPARADA PARA DESCARGA");
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "NO SE CARGÓ CORRECTAMENTE LA NOTIFICACIÓN_LICENCIA");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
        ...
        if (!selectedNotificaciones.isEmpty()) {
            if (tipoAbandono != null && !tipoAbandono.trim().isEmpty()) {
                ...
                for (int i = 0; i < selectedNotificaciones.size(); i++) {
                    ...
                    int nextabandono = c.getNextNumeroAbandonoLicencia(new Date());
                    ...
                    if (!c.updateLicenciaUso(notaux)) {
                        ...
                        c.saveHistorial("ABANDONO_LIC", "NOTIFICADAS_LIC", notaux.getSolicitud(), "PASADO A", loginBean.getUsuario().getId(), loginBean.getNombre());
                        n++;
        ...
        if (notificacion != null) {
            if (notificacion.getRegistro() != null && !notificacion.getRegistro().trim().isEmpty()) {
                if (notificacion.getSolicitante() != null && !notificacion.getSolicitante().trim().isEmpty()) {
                    if (notificacion.getApoderadoRepresentante() != null && !notificacion.getApoderadoRepresentante().trim().isEmpty()) {
                        if (notificacion.getNotificacion() != null) {
                            loginBean.setLicencia(notificacion);
                            loginBean.setVarious(false);
                            loginBean.setTipoTramite(4);
                            PrimeFaces.current().ajax().addCallbackParam("doit", true);
    ...
    public void downloadSelectedErjafe(ActionEvent ae) {
        ...
        if (selectedNotificaciones != null && !selectedNotificaciones.isEmpty()) {
            ...
            String msj = "";
            for (int i = 0; i < selectedNotificaciones.size(); i++) {
                LicenciaUso notificadaaux = selectedNotificaciones.get(i);
                ...
                if (notificadaaux.getApoderadoRepresentante() == null || notificadaaux.getApoderadoRepresentante().trim().isEmpty()) {
            ...
            if (flag) {
                ...
                loginBean.setTipoTramite(4);
                PrimeFaces.current().ajax().addCallbackParam("doit", true);
                System.out.println("envía notificada licencia uso descargar");
     ...
     */
    public List<LicenciaUso> getNotificaciones() {
     ...
     */
    public void setNotificaciones(List<LicenciaUso> notificaciones) {
     ...
     */
    public List<LicenciaUso> getNotificacionesFiltradas() {
     ...
     */
    public void setNotificacionesFiltradas(List<LicenciaUso> notificacionesFiltradas) {
     ...
     */
    public LicenciaUso getNotificacion() {
        return notificacion;
    }
     ...
     */
    public void setNotificacion(LicenciaUso notificacion) {
     ...
     */
    public List<LicenciaUso> getSelectedNotificaciones() {
     ...
     */
    public void setSelectedNotificaciones(List<LicenciaUso> selectedNotificaciones) {
