 ...
 * @author micharesp
 */
@ManagedBean(name = "notificacionPrendaBean")
@ViewScoped
public class NotificacionPrendaBean implements Serializable {
    ...
    private List<PrendaComercial> notificaciones;
    private List<PrendaComercial> notificacionesFiltradas;
    ...
    private PrendaComercial notificacion;
    ...
    private List<PrendaComercial> selectedNotificaciones;
    ...
    public NotificacionPrendaBean() {
        loadNotificacionesPrendaComercial();
    }

    private void loadNotificacionesPrendaComercial() {
        Controlador c = new Controlador();
        notificaciones = c.getPrendasComercialesNotificada("NOTIFICADA");
    ...
    public void buscarNotificacionesPrendaComercial(ActionEvent ae) {
        FacesMessage msg = null;
        if (criterio.trim().isEmpty()) {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "INGRESE UN CRITERIO DE BÚSQUEDA VÁLIDO");
        } else {
            Controlador c = new Controlador();
            notificaciones = c.getPrendasComercialesByCriteriaAndType(criterio, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
    ...
    public void buscarNotificacionesPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            notificaciones = c.getPrendasComercialesByFechaAndType(fechaInicio, fechaFin, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarNotificacionesPorFechaCertificado(ActionEvent ae) {
        ...
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            notificaciones = c.getPrendasComercialesByFechaNotificacionAndType(fechaInicioCertificado, fechaFinCertificado, "NOTIFICADA");
            numRegistros = "Número Registros Mostrados: " + notificaciones.size();
            if (notificaciones.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarNotificacion(ActionEvent ae) {
        FacesMessage msg = null;
        notificacion = (PrendaComercial) notificacionesDataTable.getRowData();
        if (notificacion != null) {
            Controlador c = new Controlador();
            if (c.removePrendaComercial(notificacion)) {

                c.saveHistorial("NOTIFICADA_PRENDA", "NOTIFICADA_PRENDA ", notificacion.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadNotificacionesPrendaComercial();
...
//            System.out.println("fechaaaaaaaaaA: " + notificacion.getFechaPresentacion());
            roselectable = false;
            dialogTitle = "EDITAR NOTIFICACIÓN PRENDA COMERCIAL " + notificacion.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Notificación_Prenda: " + notificacion.getSolicitud() + "?";
            Controlador c = new Controlador();
            RenewalForm rf = c.getRenewalFormsByApplicationNumber(notificacion.getSolicitud());
            if (rf.getId() != null) {
                notificacion.setIdRenewalForm(rf.getId());
            }
            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICACIÓN_PRENDA CARGADA.");
            PrimeFaces.current().ajax().addCallbackParam("peditar", true);
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "PROBLEMA AL CARGAR NOTIFICACIÓN_PRENDA");
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVA NOTIFICACIÓN PRENDA COMERCIAL";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Notificación_Prenda?";
        notificacion = new PrendaComercial();
        notificacion.setTipoEstado("NOTIFICADA");
        Controlador c = new Controlador();
        notificacion.setNotificacion(c.getNextNumeroNotificacionPrendaComercial(new Date()));
    ...
    public void validarNotificacion(PrendaComercial cambion) {
        FacesMessage msg = null;
        if (cambion != null) {
            ...
            if (notificacion.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    if (estadoTemp.equals("CERTIFICADO")) {
                        ...
                        notificacion.setFechaPrenda(Operaciones.cambiarFechaADiaDado(new Date(), 5));
                        notificacion.setPrendaNo(c.getNextPrendaComercialPrendaNo());
                    } else if (estadoTemp.equals("DESISTIDAS")) {
                        notificacion.setTipoEstado("DESISTIDA");
                    } else if (estadoTemp.equals("CADUCADAS")) {
                        notificacion.setResolucionCaducada(c.getNextPrendaComercialResolucionCaducada());
                        ...
                        notificacion.setTipoEstado("LEVANTAMIENTO");
                        notificacion.setLevantamientoPrendaNo(c.getNextNumeroLevantamientoPrenda(new Date()));
                    }

                    if (c.updatePrendaComercial(notificacion)) {
                        c.saveHistorial(notificacion.getTipoEstado() + "_PRENDA", "NOTIFICADA_PRENDA", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadNotificacionesPrendaComercial();
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NOTIFICADA_PRENDA EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA NOTIFICADA_PRENDA");
                    }

                } else {
                    //Editar Notificacion
                    if (c.validarExistenciaPrendaComercial(notificacion)) {
                        ...
                        if (c.updatePrendaComercial(notificacion)) {
                            c.saveHistorial("NOTIFICADA_PRENDA", "NOTIFICADA_PRENDA", notificacion.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                ...
                //Guardar Prenda Comercial notificación
                if (c.existePrendaComercial(notificacion.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        notificacion.setSolicitud(notificacion.getSolicitud().toUpperCase());
                        notificacion.setTipoEstado("NOTIFICADA");
                        if (c.savePrendaComercial(notificacion)) {
                            c.saveModificacionApp(notificacion.getDenominacion(), notificacion.getRegistro(), notificacion.getSolicitud(), "PRENDA COMERCIAL", loginBean.getNombre());
                            c.saveHistorial("NOTIFICADA_PRENDA", "NOTIFICADA_PRENDA", notificacion.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadNotificacionesPrendaComercial();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
...
//        System.out.println("Algo tiene que hacer aquí");
        if (notificacion != null) {
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (notificacion.getNotificacion() != null && notificacion.getNotificacion() != 0) {
                    if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {
                        ...
                        c.updatePrendaComercial(notificacion);

                        System.out.println("Descargando Notificación_prenda: " + notificacion.getSolicitud());

                        loginBean.setVarious(false);
                        loginBean.setPrendaComercial(notificacion);

                        PrimeFaces.current().ajax().addCallbackParam("doit", true);

                        System.out.println("envía notificación_cn descargar");
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "DESCARGANDO NOTIFICACIÓN_Prenda " + notificacion.getSolicitud());
                    } else {
            ...
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "VALIDE QUE LOS DATOS DE LA NOTIFICACIÓN_PRENDA SEAN CORRECTOS");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
            ...
            if (!c.validarDelegadoActivo("delegado")) {
                ...
                if (notificacion.getNotificacion() != null && notificacion.getNotificacion() != 0) {
                    if (!c.getRosBySolicitud(notificacion.getSolicitud()).isEmpty()) {
                        ...
                        System.out.println("envía notificación_prenda descargar");
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (notificacion != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: NOTIFICADA_PRENDA";
            }
        }
    }
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
                                        ...
                                        if (hf.getId() != null) {
                                            ...
                                            if (notificacion.getRegistro() != null && !notificacion.getRegistro().trim().isEmpty() && notificacion.getFechaRegistro() == null) {
                                    ...
                                    if (apoder.getId() != null) {
                                        notificacion.setTitApodRepre(c.getNamesPersonRenewalTextTypeByIdRenewal(rf.getId(), "AGENT"));
                                    }

                                    Person acreed = c.getFirstPersonRenewalTypeByIdRenewal(rf.getId(), "BENEFICIARY");
                                    if (acreed.getId() != null) {
                                        notificacion.setPrendariaAcreedora(acreed.getName());
                                    }
                                    Person deudP = c.getFirstPersonRenewalTypeByIdRenewal(rf.getId(), "APPLICANT");
                                    if (deudP.getId() != null) {
                                        notificacion.setDeudoraPrendaria(deudP.getName());
                                    }
                                    if ((notificacion.getRegistro() == null || notificacion.getRegistro().trim().isEmpty()) && rf.getTransactionNumber() != null && !rf.getTransactionNumber().trim().isEmpty()) {
                                        notificacion.setRegistro(rf.getTransactionNumber());
                                    }
...
//      RequestContext context = RequestContext.getCurrentInstance();
        if (!selectedNotificaciones.isEmpty()) {
            ...
            if (band) {
                System.out.println("Descargando Múltiples Notificaciones_prenda...");
                loginBean.setVarious(true);
                loginBean.setPrendasComerciales(selectedNotificaciones);
    ...
    private String getSolicitudesParaProrroga() {
        String paraProrroga = "";
        for (int i = 0; i < selectedNotificaciones.size(); i++) {
            PrendaComercial notaux = selectedNotificaciones.get(i);
    ...
    public void prepararParaProrrogas() {
        ...
        if (selectedNotificaciones.isEmpty()) {
            ...
            for (int i = 0; i < selectedNotificaciones.size(); i++) {
                PrendaComercial notaux = selectedNotificaciones.get(i);
                if (!notaux.getNotificacionEmitida()) {
        ...
        if (!selectedNotificaciones.isEmpty()) {
            if (tipoAbandono != null && !tipoAbandono.trim().isEmpty()) {
                ...
                for (int i = 0; i < selectedNotificaciones.size(); i++) {
                    ...
                    int nextabandono = c.getNextNumeroAbandonoPrenda(new Date());
    ...
    public String getTooltipAbandono(PrendaComercial noti) {
        ...
        if (notificacion != null) {
            if (notificacion.getRegistro() != null && !notificacion.getRegistro().trim().isEmpty()) {
                if (notificacion.getSolicitante() != null && !notificacion.getSolicitante().trim().isEmpty()) {
                    if (notificacion.getTitApodRepre() != null && !notificacion.getTitApodRepre().trim().isEmpty()) {
                        if (notificacion.getNotificacion() != null) {
                            loginBean.setPrendaComercial(notificacion);
                            loginBean.setTipoTramite(5);
                            loginBean.setVarious(false);
                            PrimeFaces.current().ajax().addCallbackParam("doit", true);

                            System.out.println("envía notificada prenda descargar");
    ...
    public void downloadSelectedErjafe(ActionEvent ae) {
        ...
        if (selectedNotificaciones != null && !selectedNotificaciones.isEmpty()) {
            ...
            String msj = "";
            for (int i = 0; i < selectedNotificaciones.size(); i++) {
                PrendaComercial notificadaaux = selectedNotificaciones.get(i);
                ...
                if (notificadaaux.getTitApodRepre()== null || notificadaaux.getTitApodRepre().trim().isEmpty()) {
            ...
            if (flag) {
                ...
                loginBean.setTipoTramite(5);
                PrimeFaces.current().ajax().addCallbackParam("doit", true);
                System.out.println("envía notificada prendas descargar");
     ...
     */
    public List<PrendaComercial> getNotificaciones() {
     ...
     */
    public void setNotificaciones(List<PrendaComercial> notificaciones) {
     ...
     */
    public List<PrendaComercial> getNotificacionesFiltradas() {
     ...
     */
    public void setNotificacionesFiltradas(List<PrendaComercial> notificacionesFiltradas) {
     ...
     */
    public PrendaComercial getNotificacion() {
        return notificacion;
    }
     ...
     */
    public void setNotificacion(PrendaComercial notificacion) {
     ...
     */
    public List<PrendaComercial> getSelectedNotificaciones() {
     ...
     */
    public void setSelectedNotificaciones(List<PrendaComercial> selectedNotificaciones) {
