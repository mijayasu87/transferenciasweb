 ...
 * @author Michael
 */
@ManagedBean(name = "caducadaLicenciaBean")
@ViewScoped
public class CaducadaLicenciaBean implements Serializable {
    ...
    private List<LicenciaUso> caducadas;
    private List<LicenciaUso> caducadasFiltradas;
    ...
    private List<LicenciaUso> selectedCaducadas;

    private String estadoTemp;

    public CaducadaLicenciaBean() {
        loadCaducadasLicencia();

    }

    private void loadCaducadasLicencia() {
        Controlador c = new Controlador();
        caducadas = c.getLicenciasUsoByTipo("CADUCADA");
    ...
    public void buscarCaducada(ActionEvent ae) {
        ...
        if (criterio.trim().isEmpty()) {
            ...
            Controlador c = new Controlador();
            caducadas = c.getLicenciasUsoByCriteriaAndType(criterio.trim(), "CADUCADA");
            numRegistros = "Número Registros Mostrados: " + caducadas.size();
    ...
    public void buscarCaducadasPorFecha(ActionEvent ae) {
        ...
        if (validarFechas()) {
            Controlador c = new Controlador();
            caducadas = c.getLicenciasUsoByFechaAndType(fechaInicio, fechaFin, "CADUCADA");
            numRegistros = "Número Registros Mostrados: " + caducadas.size();
            if (caducadas.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarCaducada(ActionEvent ae) {
        FacesMessage msg = null;
        caducada = (LicenciaUso) caducadaDataTable.getRowData();
        if (caducada != null) {
            Controlador c = new Controlador();
            if (c.removeLicenciaUso(caducada)) {
                c.saveHistorial("CADUCADA_LICENCIA", "CADUCADA_LICENCIA", caducada.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadCaducadasLicencia();
                System.out.println("Caducada_cn " + caducada.getSolicitud() + " Eliminada");
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CADUCADA_LICENCIA " + caducada.getSolicitud() + "ELIMINADA");
            } else {
                msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL ELIMINAR CADUCADA_LIC");
            }
        } else {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "PROBLEMA AL CARGAR CADUCADA_LIC");
        }
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
    ...
    public void prepararEditar(ActionEvent ae) {
        ...
        caducada = (LicenciaUso) caducadaDataTable.getRowData();
        if (caducada != null) {
            dialogTitle = "EDITAR CADUCADA-NEGADA LICENCIA " + caducada.getSolicitud();
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO CADUCADA-NEGADO LICENCIA";
        saveEdit = "GUARDAR";
        estadoTemp = null;
        ...
        caducada.setResolucionNo(c.getNextLicenciaUsoResolucionCaducadaNo("CADUCADA"));
        edicion = false;
        if (caducada != null) {
        ...
        if (caducada != null) {
            ...
            if (caducada.getId() != null) {
                if (estadoTemp != null && estadoTemp.equals("NOTIFICADA")) {
                    ...
                    if (c.updateLicenciaUso(caducada)) {
                        c.saveHistorial("NOTIFICADA_LICENCIA", "CADUCADA_LICENCIA", caducada.getSolicitud(), "PASADO A", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadCaducadasLicencia();
                ...
                } else if (c.validarExistenciaLicenciaUso(caducada)) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    caducada.setSolicitud(caducada.getSolicitud().toUpperCase());
                    if (c.updateLicenciaUso(caducada)) {
                        c.saveHistorial("CADUCADA_LICENCIA", "CADUCADA_LICENCIA", caducada.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                        PrimeFaces.current().ajax().addCallbackParam("saved", true);
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CADUCADA_LICENCIA EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR EL CADUCADA_LICENCIA");
                    }
                }

            } else {
                //Guardar Caducada
                if (c.existeLicenciaUso(caducada.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        caducada.setSolicitud(caducada.getSolicitud().toUpperCase());
                        caducada.setTipoEstado("CADUCADA");
                        caducada.setResolucionCaducada(c.getNextLicenciaUsoResolucionCaducadaNo("CADUCADA"));
                        if (c.saveLicenciaUso(caducada)) {
                            c.saveModificacionApp(caducada.getDenominacion(), caducada.getRegistro(), caducada.getSolicitud(), "LICENCIA DE USO", loginBean.getNombre());
                            loadCaducadasLicencia();
                            c.saveHistorial("CADUCADA_LICENCIA", "CADUCADA_LICENCIA", caducada.getSolicitud(), "NUEVO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CADUCADA_LICENCIA GUARDADO CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL GUARDAR EL CADUCADA_LICENCIA");
                        }
                    }
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (caducada != null) {
            ...
            if (historial.trim().isEmpty()) {
                historial = "Estado actual: CADUCADA-NEGADA LICENCIA";
            }
        }
    }
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
                                        ...
                                        if (apoder.getId() != null) {
                                            caducada.setApoderadoRepresentante(c.getNamesPersonRenewalTextTypeByIdRenewal(rf.getId(), "AGENT"));
                                        }
                                    ...
                                    msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "TRÁMITE ENCONTRADO PERO NO ES UNA LICENCIA DE USO, SINO " + t.getName());
                                }
                            } else {
        ...
        if (caducada != null) {
            ...
            if (caducada.getRegistro() != null && !caducada.getRegistro().trim().isEmpty()) {
                ...
                if (caducada.getSolicitante() == null || caducada.getSolicitante().trim().isEmpty()) {
                    ...
                    correcto = true;
                    loginBean.setLicencia(caducada);
                    loginBean.setVarious(false);
                    loginBean.setTipoTramite(4);//3: para licencias de uso
                    PrimeFaces.current().ajax().addCallbackParam("doit", true);
    ...
    public void downloadSelected(ActionEvent ae) {
        ...
        if (!selectedCaducadas.isEmpty()) {
            ...
            System.out.println("Descargando Múltiples Caducadas Lic... " + selectedCaducadas.size());

            boolean flag = true;
            Controlador c = new Controlador();
            String msj = "";            
            for (int i = 0; i < selectedCaducadas.size(); i++) {                
                LicenciaUso caducadaaux = selectedCaducadas.get(i);
            ...
            if (flag) {
                loginBean.setLicencias(selectedCaducadas);
                loginBean.setVarious(true);
                loginBean.setTipoTramite(4);//3: para cambios nombre                
                PrimeFaces.current().ajax().addCallbackParam("doit", true);
     ...
     */
    public List<LicenciaUso> getCaducadas() {
        return caducadas;
    }
     ...
     */
    public void setCaducadas(List<LicenciaUso> caducadas) {
     ...
     */
    public List<LicenciaUso> getCaducadasFiltradas() {
     ...
     */
    public void setCaducadasFiltradas(List<LicenciaUso> caducadasFiltradas) {
     ...
     * @return the caducada
     */
    public LicenciaUso getCaducada() {
        return caducada;
    }
     ...
     */
    public void setCaducada(LicenciaUso caducada) {
     ...
     */
    public List<LicenciaUso> getSelectedCaducadas() {
     ...
     */
    public void setSelectedCaducadas(List<LicenciaUso> selectedCaducadas) {
