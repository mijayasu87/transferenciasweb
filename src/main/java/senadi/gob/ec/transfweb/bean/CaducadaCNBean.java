            ...
            if (caducada.getId() != null) {
                if (estadoTemp != null && estadoTemp.equals("NOTIFICADA")) {
                    ...
                    if (c.updateCambioNombre(caducada)) {
                        c.saveHistorial("NOTIFICADA_CN", "CADUCADA_CN", caducada.getSolicitud(), "PASADO A", loginBean.getUsuario().getId(), loginBean.getNombre());
                        loadCaducadasCN();
                ...
                } else if (c.validarExistenciaCambioNombre(caducada)) {
