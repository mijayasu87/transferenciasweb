 ...
 * @author micharesp
 */
@ManagedBean(name = "certificadocnBean")
@ViewScoped
public class CertificadoCNBean implements Serializable {
    ...
    private List<CambioNombre> certificados;
    private List<CambioNombre> certificadosFiltradas;
    ...
    private CambioNombre certificado;
    ...
    private List<CambioNombre> selectedCertificados;
    ...
    public CertificadoCNBean() {
        loadCertificadosCN();
    }

    private void loadCertificadosCN() {
        Controlador c = new Controlador();
        certificados = c.getCambiosNombreByTipo("CERTIFICADO");
        numRegistros = "Número Registros Mostrados: " + certificados.size();
        exportName = "certificado_cn_" + Operaciones.formatDate(new Date());
...
//                                    System.out.println("debug_id: "+rf.getDebugId());
    ...
    public void validarCertificado(CambioNombre cambi) {
        FacesMessage msg = null;
        if (cambi != null) {
    ...
    public void buscarCertificadosCN(ActionEvent ae) {
        FacesMessage msg = null;
        if (criterio.trim().isEmpty()) {
            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "INGRESE UN CRITERIO DE BÚSQUEDA VÁLIDO");
        } else {
            Controlador c = new Controlador();
            certificados = c.getCambiosNombreByCriteriaAndType(criterio.trim(), "CERTIFICADO");
            numRegistros = "Número Registros Mostrados: " + certificados.size();
    ...
    public void buscarCertificadoCNPorFecha(ActionEvent ae) {
        FacesMessage msg = null;
        if (validarFechas()) {
            Controlador c = new Controlador();
            certificados = c.getCambiosNombreByFechaAndType(fechaInicio, fechaFin, "CERTIFICADO");
            numRegistros = "Número Registros Mostrados: " + certificados.size();
            if (certificados.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void buscarCertificadoCNPorFechaCertificado(ActionEvent ae) {
        FacesMessage msg = null;
        if (validarFechasCertificado()) {
            Controlador c = new Controlador();
            certificados = c.getCambiosNombreByFechaCertificadoAndType(fechaInicioCertificado, fechaFinCertificado, "CERTIFICADO");
            numRegistros = "Número Registros Mostrados: " + certificados.size();
            if (certificados.isEmpty()) {
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "NO SE ENCONTRARON RESULTADOS");
            } else {
    ...
    public void eliminarCertificadoCN(ActionEvent ae) {
        FacesMessage msg = null;
        certificado = (CambioNombre) certificadosDataTable.getRowData();
        if (certificado != null) {
            Controlador c = new Controlador();
            if (c.removeCambioNombre(certificado)) {
                c.saveHistorial("CERTIFICADO_CN", "CERTIFICADO_CN ", certificado.getSolicitud(), "ELIMINADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                loadCertificadosCN();
                System.out.println("Certificado_cn " + certificado.getSolicitud() + " Eliminada");
                msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CERTIFICADO_CN " + certificado.getSolicitud() + " ELIMINADA");
            } else {
                msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "HUBO UN PROBLEMA AL ELIMINAR CERTIFICADO_CN");
            }
        } else {
    ...
    public void prepararEditar(ActionEvent ae) {
        ...
        certificado = (CambioNombre) certificadosDataTable.getRowData();
        if (certificado != null) {
            dialogTitle = "EDITAR CERTIFICADO CAMBIO DE NOMBRE " + certificado.getSolicitud();
            mensajeConfirmacion = "¿Seguro de editar la Certificado_cn: " + certificado.getSolicitud() + "?";
            Controlador c = new Controlador();
    ...
    public void prepararNuevo(ActionEvent ae) {
        dialogTitle = "NUEVO CERTIFICADO CAMBIO DE NOMBRE";
        saveEdit = "GUARDAR";
        mensajeConfirmacion = "¿Seguro de guardar la Nueva Certificado_cn?";
        certificado = new CambioNombre();
...
//        certificado.setCertificado(c.getNextCambioNombreCertificado());
    ...
    public void guardarCertificadoCN(ActionEvent ae) {
        FacesMessage msg = null;
        if (certificado != null) {
            Controlador c = new Controlador();
            if (certificado.getId() != null) {
                if (estadoTemp != null && !estadoTemp.trim().isEmpty()) {
                    System.out.println("tipo_estado elegido: " + estadoTemp + ", estaba en certificado_cn");
                    if (estadoTemp.equals("NOTIFICADAS")) {
                        certificado.setNotificacion(c.getNextNumeroNotificacionCN(new Date()));
                    ...
                    if (c.updateCambioNombre(certificado)) {
                        ...
                        msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CERTIFICADO_CN EDITADA CON ÉXITO");
                    } else {
                        msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA CERTIFICADO_CN");
                    }

                } else {
                    //Editar CertificadoCD
                    if (c.validarExistenciaCambioNombre(certificado)) {
                        ...
                        if (c.updateCambioNombre(certificado)) {
                            c.saveHistorial("CERTIFICADO_CN", "CERTIFICADO_CN", certificado.getSolicitud(), "EDITADO", loginBean.getUsuario().getId(), loginBean.getNombre());
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CERTIFICADO_CN EDITADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL EDITAR LA CERTIFICADO_CN");
...
//                //Guardar Certificado_CN
                if (c.existeCambioNombre(certificado.getSolicitud())) {
                    msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "YA EXISTE UN REGISTRO CON EL MISMO NÚMERO DE SOLICITUD INGRESADO");
                } else {
                    ...
                    if (habilitado) {
                        ...
                        certificado.setSolicitud(certificado.getSolicitud().toUpperCase());
                        certificado.setTipoEstado("CERTIFICADO");
                        if (c.saveCambioNombre(certificado)) {
                            c.saveModificacionApp(certificado.getDenominacion(), certificado.getRegistro(), certificado.getSolicitud(), "CAMBIO DE NOMBRE", loginBean.getNombre());
                            c.saveHistorial("CERTIFICADO_CN", "CERTIFICADO_CN", certificado.getSolicitud(), "NUEVA", loginBean.getUsuario().getId(), loginBean.getNombre());
                            loadCertificadosCN();
                            PrimeFaces.current().ajax().addCallbackParam("saved", true);
                            msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "INFORMACIÓN", "CERTIFICADO_CN GUARDADA CON ÉXITO");
                        } else {
                            msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "INFORMACIÓN", "HUBO UN PROBLEMA AL GUARDAR EL CERTIFICADO CD");
                        }
                    }
                }
            }
    ...
    public void viewCertificadoCN(ActionEvent ae) {
        FacesMessage msg = null;
        certificado = (CambioNombre) certificadosDataTable.getRowData();
        ...
        if (certificado != null) {
            ...
            if (certificado.getRegistro() != null && !certificado.getRegistro().trim().isEmpty()) {
                System.out.println("Descargando Certificado_CN: " + certificado.getSolicitud());
                ...
                if (!c.validarDelegadoActivo("delegado")) {
                    ...
                    correcto = true;
//                    loginBean.setTransferenciaFlotante(certificado); arrreglar aquí
...
//                    loginBean.setNotificacionesFlotantes(new ArrayList<Notificacion>());
                    ...
                    loginBean.setCambioNombre(certificado);
    ...
    public void prepararHistorial(ActionEvent ae) {
        ...
        if (certificado != null) {
            ...
            if (getHistorial().trim().isEmpty()) {
                historial = "Estado actual: CERTIFICADO_CN";
            }
        }
    }
...
//        RequestContext context = RequestContext.getCurrentInstance();
        if (!selectedCertificados.isEmpty()) {
            ...
            System.out.println("Descargando Múltiples Certificados_cn...");
            ...
            if (band) {
                ...
                loginBean.setCambiosNombre(selectedCertificados);
...
//            System.out.println("No hay seleccionadas");
    ...
    public void verNotificacionCambioNombre(ActionEvent ae) {
        FacesMessage msg = null;
        certificado = (CambioNombre) certificadosDataTable.getRowData();
        if (certificado != null) {
            ...
            if (uploads.isEmpty()) {
                ...
                if (!rutaCertCasillero.trim().isEmpty()) {
                    ...
                    msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "CAMBIO DE NOMBRE", "VISUALIZANDO CERTIFICADO");
                } else {
                    msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "AVISO", "NO SE ENCUENTRA UN CERTIFICADO REALIZADO PARA ESTE TRÁMITE");
                }
            }
     ...
     */
    public List<CambioNombre> getCertificados() {
        return certificados;
    }
     ...
     */
    public void setCertificados(List<CambioNombre> certificados) {
     ...
     */
    public List<CambioNombre> getCertificadosFiltradas() {
     ...
     */
    public void setCertificadosFiltradas(List<CambioNombre> certificadosFiltradas) {
     ...
     */
    public CambioNombre getCertificado() {
        return certificado;
    }
     ...
     */
    public void setCertificado(CambioNombre certificado) {
     ...
     */
    public List<CambioNombre> getSelectedCertificados() {
     ...
     */
    public void setSelectedCertificados(List<CambioNombre> selectedCertificados) {
